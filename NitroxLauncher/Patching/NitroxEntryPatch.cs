...
namespace NitroxLauncher.Patching
{
    internal class NitroxEntryPatch
    {
        ...
        private const string PLATFORM_UTILS_TYPE_NAME = "PlatformUtils";
        private const string PLATFORM_UTILS_METHOD_NAME = "PlatformInitAsync";
        ...
        public void Apply()
        {
            ...
            using (ModuleDefMD nitroxPatcherAssembly = ModuleDefMD.Load(nitroxPatcherPath))
            {
                ...
                TypeDef gameInputType = module.GetTypes().First(x => x.FullName == PLATFORM_UTILS_TYPE_NAME);
                MethodDef awakeMethod = gameInputType.Methods.First(x => x.Name == PLATFORM_UTILS_METHOD_NAME);
